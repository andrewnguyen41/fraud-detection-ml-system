...
from .s3_utils import (download_data_from_s3, download_model_from_s3,
                       getS3Client, save_to_s3)
...
def load_model():
    ...
    if os.path.exists(modelPath):
        if latest_train_timestamp == None:
            ...
            last_modified_date = datetime.fromtimestamp(modification_time).strftime(
                "%Y%m%d_%H%M%S"
            )
...
def retrain_model():
    ...
    for file in csv_files:
        ...
        modified_time = datetime.fromtimestamp(os.path.getmtime(file_path)).strftime(
            "%Y%m%d_%H%M%S"
        )
...
def check_for_new_data():
    ...
    for obj in response.get("Contents", []):
        ...
        if file_key.startswith("creditcard_"):
            ...
            if (
                latest_train_timestamp is None
                or file_timestamp > latest_train_timestamp
            ):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(download_data_from_s3, file_key): file_key
